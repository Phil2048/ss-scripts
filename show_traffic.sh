...
#echo "-----------------------账户信息-----------------------"
mysql -u$user -p$password $database -e "select dat_date,port,uname,concat(format(upload/1024/1024,0),'M') as upload,concat(format(download/1024/1024,0),'M') as download from $database.$hist_table order by port, dat_date"
echo 
...
select 'all','---','---','---',concat(format(sum(upload)/1024/1024,0),'M'),concat(format(sum(download)/1024/1024,0),'M'),'---','---','---' from $database.$table order by cast(port as signed int);"
date '+%Y%m%d %H:%M:%S'
